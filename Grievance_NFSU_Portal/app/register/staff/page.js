...
import { FaUser, FaEnvelope, FaLock, FaPhone, FaSpinner, FaSun, FaMoon, FaUserShield } from 'react-icons/fa';
...
export default function StaffRegisterPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post('/api/auth/register', {
                ...
                role: 'staff', // Registering as 'staff'
            });

            if (response.data.success) {
                localStorage.setItem('token', response.data.token);
                localStorage.setItem('user', JSON.stringify(response.data.user));
                // Redirect to the existing admin/staff dashboard
    ...
    return (
        <div className="min-h-screen relative overflow-hidden flex items-center justify-center py-12 bg-slate-50 dark:bg-[#0f172a] transition-colors duration-500">
            ...
            <button
                onClick={toggleTheme}
                className="fixed top-6 right-6 p-3 glass-theme rounded-full text-slate-500 dark:text-gray-400 hover:text-purple-500 dark:hover:text-purple-400 transition-all z-50 border border-slate-200 dark:border-white/10 shadow-lg"
            >
                ...
                <div className="glass-card-theme p-8 md:p-10 border border-slate-200 dark:border-white/10 shadow-2xl">
                    ...
                    <div className="text-center mb-8">
                        <div className="relative inline-block mb-4">
                            ...
                            />
                            <div className="absolute -bottom-2 -right-2 bg-purple-600 p-2 rounded-lg text-white shadow-lg">
                                <FaUserShield className="text-sm" />
                            </div>
                        </div>
                        <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-2">Staff Registration</h1>
                        <p className="text-slate-600 dark:text-gray-300 font-medium">Create a Staff Account</p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        required
                                        className="w-full pl-12 pr-4 py-3 bg-slate-900/5 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all font-medium"
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full py-3 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-lg font-semibold hover:shadow-lg hover:shadow-purple-500/50 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
                        >
                            {loading ? (
                                <>
                    ...
                    <div className="mt-8 text-center border-t border-slate-200 dark:border-white/10 pt-6">
                        ...
                        <Link href="/register-selection" className="block mt-4 text-slate-500 dark:text-gray-400 hover:text-purple-500 dark:hover:text-purple-400 transition-colors font-semibold">
