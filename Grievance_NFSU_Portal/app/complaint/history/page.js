    ...
    const fetchHistory = async (token) => {
        try {
            ...
            if (response.data.success) {
                // Filter resolved only and sort by resolvedAt descending
                    ...
                    >
                        <div>
                            ...
                            <h1 className="text-4xl font-extrabold text-slate-900 dark:text-white flex items-center gap-4">
                                <div className="p-3 bg-green-500/20 text-green-500 rounded-2xl border border-green-500/30">
                        ...
                        <div className="flex gap-4">
                            <div className="relative group flex-1 md:w-80">
                                ...
                                <input
                                    ...
                                    className="w-full pl-12 pr-4 py-3.5 bg-white/10 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl text-slate-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all font-medium backdrop-blur-md"
                                />
                            </div>
                        </div>
                    </motion.div>
                            ...
                            <div className="grid grid-cols-1 gap-6">
                                {filteredComplaints.map((complaint, index) => (
                                    <motion.div
                                        ...
                                        className="glass-card-theme p-6 group hover:bg-white/10 dark:hover:bg-white/10 transition-all cursor-pointer border-slate-200 dark:border-white/10 shadow-xl overflow-hidden relative"
                                    >
                                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                            <div className="flex-1">
                                                <div className="flex items-center gap-3 mb-2">
                                                    <span className="text-[10px] uppercase tracking-widest font-black px-2 py-1 bg-blue-500/10 text-blue-500 rounded-md border border-blue-500/20">
                                                        {complaint.department}
                                                    </span>
                                                    <span className="text-[10px] uppercase tracking-widest font-black px-2 py-1 bg-purple-500/10 text-purple-500 rounded-md border border-purple-500/20">
                                                        {complaint.category}
                                                    </span>
                                                </div>
                                                <h2 className="text-xl font-bold text-slate-900 dark:text-white group-hover:text-blue-500 transition-colors mb-1">
                                                    {complaint.title}
                                                </h2>
                                            ...
                                            <div className="flex items-center gap-6">
                                                {complaint.resolutionDetails?.proof?.length > 0 && (
                                                    <div className="flex items-center gap-2 px-3 py-1.5 bg-blue-500/10 text-blue-500 rounded-xl text-xs font-bold border border-blue-500/20">
                                                        <FaImages />
