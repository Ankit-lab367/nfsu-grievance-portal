...
export default function ComplaintDetailsPage({ params }) {
    ...
    const fetchComplaint = async (token) => {
        try {
            // We use the existing get API but might need to filter by ID
            // Ideally we should have a specific endpoint or use query param
        ...
        setComments(prev => prev.map(c => {
            if (c.id === commentId) {
                ...
                if (isSameVote) {
                    ...
                    // Switching votes or first time vote
        ...
        <div className="min-h-screen relative bg-slate-50 dark:bg-[#0f172a] transition-colors duration-500">
            ...
            <div className="container mx-auto px-6 py-12 relative z-10">
                <motion.div
                    ...
                    className="max-w-4xl mx-auto glass-card-theme p-8 border-slate-200 dark:border-white/10 shadow-2xl"
                >
                    <div className="flex justify-between items-start mb-8">
                        <div>
                            <Link
                                href={(currentUser?.role === 'admin' || currentUser?.role === 'super-admin') ? "/dashboard/admin" : "/dashboard/student"}
                                className="inline-flex items-center text-blue-400 hover:text-blue-300 mb-4 transition-colors font-bold"
                            >
                                <FaArrowLeft className="mr-2" />
                                Back to {(currentUser?.role === 'admin' || currentUser?.role === 'super-admin') ? "Staff Portal" : "Dashboard"}
                            </Link>
                            <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-3 text-shadow">{complaint.title}</h1>
                            <div className="flex items-center space-x-4 text-sm text-slate-500 dark:text-gray-400">
                                ...
                                <span className="bg-blue-500/20 text-blue-300 px-3 py-1 rounded-full text-xs font-semibold border border-blue-500/30">{complaint.department}</span>
                            </div>
                        </div>
                        <StatusBadge status={complaint.status} />
                    </div>

                    <div className="bg-slate-900/5 dark:bg-white/5 rounded-2xl p-6 mb-8 text-slate-700 dark:text-gray-300 border border-slate-200 dark:border-white/5 leading-relaxed shadow-inner">
                                    ...
                                    <div key={index} className="bg-white/5 p-4 rounded-lg border border-white/10">
                                        {file.filename.match(/\.(jpg|jpeg|png|gif)$/i) ? (
                                            ...
                                            >
                                                ...
                                                <span className="text-blue-400 hover:text-blue-300 truncate">{file.filename}</span>
                                            </a>
                                        )}
                                    </div>
                                ))}
                        ...
                        <div className="space-y-4">
                            {complaint.timeline.map((entry, index) => (
                                <div key={index} className="flex gap-4">
                                    ...
                                    <div className="pb-4">
                                        <p className="text-slate-900 dark:text-white font-bold">{entry.status}</p>
                        ...
                        <div className="flex flex-col gap-4">
                            <div className="flex items-center space-x-3">
                                ...
                                <button
                                    ...
                                    className="flex items-center space-x-1.5 px-3 py-1.5 bg-blue-500/10 text-blue-500 hover:bg-blue-500/20 rounded-lg transition-colors group"
                                >
                            ...
                            {/* Administrator Resolution Buttons */}
                                ...
                                {comments.map((comment) => (
                                    <div key={comment.id} className="space-y-3">
                                        <div className="flex items-start space-x-3">
                                            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-purple-500 flex items-center justify-center text-white font-bold text-sm border-2 border-white/10 shadow-lg shrink-0 overflow-hidden">
                                                {comment.avatar ? (
                            ...
                            <form onSubmit={handleSendComment} className="p-6 border-t border-white/10 bg-white/5">
                                <div className="flex items-center space-x-3">
                                    <input
                                        ...
                                        className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all"
                                    />
                                    <button
                                        type="submit"
                                        className="p-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl transition-all shadow-lg active:scale-95"
                                    >
                                        <FaPaperPlane />
                                    </button>
                                </div>
