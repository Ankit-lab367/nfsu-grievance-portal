...
export default function CreateNoticeForm() {
    ...
    const [formData, setFormData] = useState({
        title: '',
        content: '',
        targetAudience: 'student', // Default
    });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                router.push('/dashboard/admin/notices'); // Redirect to list view
            }
        } catch (error) {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
                type="submit"
                disabled={loading}
                className="w-full bg-gradient-to-r from-blue-600 to-cyan-600 text-white font-bold py-4 rounded-xl shadow-lg hover:shadow-blue-500/30 transition-all active:scale-95 flex items-center justify-center gap-2"
            >
