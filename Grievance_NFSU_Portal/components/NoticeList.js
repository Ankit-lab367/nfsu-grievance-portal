...
export default function NoticeList() {
    ...
    const handleDismiss = async (id, e) => {
        e.stopPropagation(); // Prevent toggling expand when clicking remove
        try {
            const token = localStorage.getItem('token');
                ...
                >
                    ...
                    >
                        <div className="flex items-center gap-4">
                            <div className={`p-3 rounded-full ${notice.targetAudience === 'staff' ? 'bg-purple-500/20 text-purple-400' : 'bg-blue-500/20 text-blue-400'}`}>
                                <FaBell />
                            </div>
                            <div>
